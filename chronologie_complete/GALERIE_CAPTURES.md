...
![Dossier de prise de poste : organisation et inventaire](../cartographie_SI/01_dossier_prise_de_poste_inventaire_SI.png)

### Dossier de prise de poste — contrats, sauvegardes et attendus

![Dossier de prise de poste : contrats et attendus](../cartographie_SI/02_dossier_prise_de_poste_contrats_et_consigne.png)
...
![A-20 — Courriel de migration légitime](../capture/05_A20_courriel_migration_legitime.png)
...
![A-14 — Badge non fiable](../capture/06_A14_badge_DATACENTER_non_fiable.png)
...
![A-17 — USB paie autorisée](../capture/07_A17_USB_paie_sauvegarde_autorisee.png)
...
![A-05 — Sauvegardes](../capture/08_A05_sauvegardes_modifiees_indisponibles.png)

### 09 — A-20 : copie archivistique du courriel de migration

Cette seconde capture est conservée comme doublon documentaire de la vérification A-20 ; elle ne constitue pas un nouvel événement.

![A-20 — Copie archivée](../capture/09_A20_courriel_migration_legitime_copie.png)
...
![Décision : isoler FIN-112 sous tension](../decisions/01_isoler_FIN-112_sous_tension.png)

## 2. Acte II — ransomware, propagation et reconstitution technique

### 10 — A-09 : règle pare-feu et canal de commande — Preuve

La règle `OUT-TEMP-443` ouvre un flux non journalisé, ensuite utilisé à intervalles réguliers par FIN-112.

![A-09 — Règle firewall](../capture/10_A09_regle_firewall_C2.png)

### 11 — A-04 : métadonnées NTFS de la note MIRAGE — Preuve

Les dates contradictoires imposent de retenir la chronologie corroborée par les autres sources.

![A-04 — Métadonnées altérées](../capture/11_A04_metadonnees_MIRAGE_alterees.png)

### 12 — A-13 : inventaire de l'impact — Preuve

23 serveurs sur 40 sont chiffrés ; l'inventaire pilote le confinement et la reprise.

![A-13 — Inventaire impact](../capture/12_A13_inventaire_serveurs_impact.png)

### 13 — A-10 : corrélation SIEM — Preuve

Les événements Windows recoupent le VPN, les privilèges, l'exécution du binaire et l'effacement des traces.

![A-10 — SIEM](../capture/13_A10_SIEM_acces_svc_oasisnet.png)

### 14 — A-03 : transferts proxy de 117,8 Go — Preuve

Le proxy mesure les flux nocturnes vers une même destination externe ; le contenu exact reste à qualifier.

![A-03 — Exfiltration proxy](../capture/14_A03_proxy_exfiltration_117_8Go.png)

### 15 — A-12 : chronologie EDR de FIN-112 — Preuve

Les trois minutes d'attaque reconstituent la séquence : exécution, désactivation, lecture, C2, chiffrement et sabotage.

![A-12 — Chronologie EDR](../capture/15_A12_chronologie_EDR_FIN-112.png)

### 16 — A-13 : copie archivistique de l'inventaire

Cette seconde capture conserve le même état d'impact A-13 ; elle n'ajoute pas un nouvel événement.

![A-13 — Copie archivée](../capture/16_A13_inventaire_serveurs_impact_copie.png)
...
![Décision : isoler les segments critiques](../decisions/02_isoler_segments_critiques.png)
...
![Décision : communication interne](../decisions/03_communication_interne_rassurer_sans_detail.png)

## 3. Diversions, incidents parallèles et triage

### 17 — A-16 : pic de trafic lié à la presse — Fausse piste

Le trafic provient de visiteurs humains après un article ; le cache tient et aucun DDoS n'est établi.

![A-16 — Trafic presse légitime](../capture/17_A16_pic_trafic_presse_legitime.png)

### 18 — A-23 : fraude au président — Preuve, mais incident distinct

La demande de 480 000 MAD est bloquée. Elle est confirmée, sans être artificiellement reliée à MIRAGE.

![A-23 — BEC](../capture/18_A23_BEC_fraude_au_president.png)

### 19 — A-15 : jeton d'ex-salarié — Fausse piste

Le jeton résiduel est un défaut de gestion d'accès à corriger, mais aucune activité MIRAGE ne lui est associée.

![A-15 — Jeton résiduel](../capture/19_A15_jeton_ex_salarie_sans_lien.png)

### 20 — A-21 : scans Internet génériques — Bruit

Les SYN Censys/Shodan ne touchent pas d'actif interne et ne constituent pas une intrusion AtlasGrid.

![A-21 — Scans génériques](../capture/20_A21_scans_internet_generiques.png)

### 21 — A-25 : PsExec de maintenance — Bruit

Outil signé, utilisé dans le cadre du ticket #4502 ; aucune corrélation avec MIRAGE.

![A-25 — Maintenance PsExec](../capture/21_A25_PsExec_maintenance_autorisee.png)

### 22 — A-27 : archivage trimestriel — Bruit

Opération autorisée par le ticket #4381 : les fichiers restent lisibles et ne sont pas chiffrés.

![A-27 — Archivage autorisé](../capture/22_A27_archivage_trimestriel_autorise.png)

### 23 — Décision OT : surveillance renforcée
...
![Décision OT — capture d'origine](../capture/23_decision_OT_surveillance_renforcee.png)

![Décision OT — fiche de décision](../decisions/04_OT_surveillance_renforcee_sans_coupure.png)

### 24 — A-26 : connexion Paris — Bruit

Appareil connu, activité de messagerie normale et contexte de déplacement : aucune corrélation MIRAGE.

![A-26 — Connexion Paris](../capture/24_A26_connexion_Paris_legitime.png)

### 25 — A-40 : ticket OasisNet d'un autre client — Bruit

Le ticket mentionne `.lockb`, mais ne concerne aucun actif AtlasGrid ni les indicateurs MIRAGE.

![A-40 — Autre client](../capture/25_A40_ticket_OasisNet_autre_client.png)

### 26 — A-24 : salarié en litige — Fausse piste

Les journaux VPN, AD et badge ainsi que l'arrêt de travail écartent la personne de la fenêtre d'attaque.

![A-24 — Salarié hors de cause](../capture/26_A24_salarie_litige_hors_de_cause.png)

### 27 — A-18 : extrait initial de la revendication DarkAtlas

La première capture est conservée comme élément de veille ; elle exigeait une comparaison avec les preuves internes.

![A-18 — Extrait DarkAtlas](../capture/27_A18_revendication_DarkAtlas_extrait.png)

### 28 — A-18 : revendication DarkAtlas invalidée — Fausse piste

Les données sont publiques et les indicateurs (extension, chronologie, canal) ne correspondent pas à MIRAGE.

![A-18 — Analyse DarkAtlas](../capture/28_A18_revendication_DarkAtlas_invalidee.png)

### 29 — A-41 : assistance distante — Bruit

Session autorisée, opérateur identifié, ticket HELP-3391 et MFA valide.

![A-41 — Assistance distante](../capture/29_A41_assistance_distance_autorisee.png)

### 30 — A-22 : faux positif cryptominer — Bruit

L'alerte vise 7-Zip signé pendant une compilation nocturne ; ni minage ni lien MIRAGE ne sont établis.

![A-22 — Faux positif](../capture/30_A22_faux_positif_cryptominer.png)
...
![Décision : assureur](../decisions/05_declaration_assureur_immediate.png)
...
![Décision : presse](../decisions/06_communication_presse_repondre_version.png)
...
![Décision : client](../decisions/07_reponse_client_rester_factuel.png)
...
![Décision : Autorité](../decisions/08_notification_autorite_transparente.png)

### 31 — A-07 : publication SIROCCO — Preuve d'exposition
...
![A-07 — Leak site SIROCCO](../capture/31_A07_leaksite_SIROCCO_donnees_clients.png)
...
- Les captures 09 et 16 sont des doublons archivistiques ; la capture 27 est la revendication initiale, invalidée par la capture 28.
